// © 2024 Ali Rasheed - https://github.com/Hakikicode
...
let lastTapTime = localStorage.getItem('lastTapTime') ? new Date(localStorage.getItem('lastTapTime')) : null;
...
function saveProgress() {
  ...
  localStorage.setItem('lastTapTime', lastTapTime ? lastTapTime.toISOString() : null);
}

// Activate multiplier for a limited time
function activateMultiplier(factor, duration) {
  multiplier = factor;
  multiplierActive = true;
  coin.classList.add('boost-active');

  setTimeout(() => {
      multiplier = 10;
      multiplierActive = false;
      coin.classList.remove('boost-active');
  }, duration);
}
...
function checkStreak() {
    const now = new Date();
    if (lastTapTime && (now - lastTapTime) / (1000 * 60 * 60 * 24) < 1) {
        streakCount++;
        coinCount += streakCount * 5;
    } else {
        streakCount = 0;
    }
    lastTapTime = now;
...
// Boost earnings when button is clicked
document.getElementById('boost-btn').addEventListener('click', () => {
  if (!multiplierActive) {
      activateMultiplier(10, 10000);
      alert("10x Earnings Boost Activated!");
  }
});

function App() {
  const [isPressed, setIsPressed] = useState(false);
  const [points, setPoints] = useState(coinCount);
      ...
      <div className="w-full z-10 min-h-screen flex flex-col items-center text-white">
          ...
          <div className="w-full flex justify-between gap-2">
            ...
            <div className="flex-grow flex items-center max-w-60 text-sm">
              <div className="w-full bg-[#fad258] py-4 rounded-2xl flex justify-around">
                ...
                <div className="h-[48px] w-[2px] bg-[#fddb6d]"></div>
                <button className="flex flex-col items-center gap-1" id="boost-btn">
        ...
        <div className="flex-grow flex items-center justify-center select-none">
          <div className="relative mt-4"
            ...
            <img src='./ngcoin.png' width={300} height={300} alt="notcoin"
              draggable="false"
              style={{
                pointerEvents: 'none',
                userSelect: 'none',
